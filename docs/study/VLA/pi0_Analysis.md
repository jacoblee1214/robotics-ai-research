...
### Zero-shot 평가 (파인튜닝 없이, 정규화 점수 0~1.0)

| 태스크 | π0 | π0-small | OpenVLA | Octo |
|--------|-----|---------|--------|------|
| Bussing Easy | **0.971** | 0.443 | 0.000 | 0.043 |
| Bussing Hard | **0.875** | 0.333 | 0.000 | 0.000 |
| Shirt Folding | **1.000** | 0.500 | 0.000 | 0.000 |
| Grocery Bagging | **0.786** | 0.271 | 0.000 | 0.000 |
| Toast out of Toaster | **0.750** | 0.000 | 0.000 | 0.000 |

OpenVLA(7B), Octo(93M) 대부분 **0점** (완전 실패). π0만 즉시 동작.
π0-small(470M, VLM 사전학습 없음)도 baseline 압도 → 아키텍처 자체의 우수성.
...
|---------|---------|
| 이미지 인코더 | 14ms |
| 관측 forward pass | 32ms |
| Flow Matching 10회 | 27ms |
| **총합** | **~73ms/청크** |

20Hz 로봇에서 여유롭게 실시간 동작 가능. FAST 버전은 자기회귀 디코딩으로 ~750ms (10배 느림).

### 복잡 다단계 태스크

세탁물 접기, 모바일 세탁, 테이블 정리, 박스 조립, 달걀 포장 등 7개 장기 태스크에서
**모두 최대 점수의 50% 이상** 달성.
...
**2단계 훈련 파이프라인:**
1. FAST 토크나이저로 이종 데이터 통합 사전학습
2. Action Expert(Flow Matching) 추가, α=10.0으로 80k 스텝 post-training

**실제 가정 환경 평가 (훈련 미노출 3곳):**

| 조건 | Follow Rate | Success Rate |
|------|------------|-------------|
| In-Distribution | 86% | 83% |
| **Out-of-Distribution (새로운 가정)** | **94%** | **94%** |

OOD 성능이 오히려 더 높음 → 일반화 능력 입증.

**Co-training Ablation (어떤 데이터가 중요한가):**

| 제거된 데이터 | OOD Success | 하락폭 |
|------------|------------|--------|
| 전체 (π0.5) | **94%** | - |
| ME(다환경) 제거 | 31% | **-63%p** ← 가장 치명적 |
| CE(크로스임바디먼트) 제거 | 49% | -45%p |
| WD(웹데이터) 제거 | 74% | -20%p |

→ 다양한 가정환경 로봇 데이터(ME)가 OOD 일반화의 핵심.
→ 웹 데이터는 새로운 물체 인식에 특히 중요.
...
**FAST = Frequency-space Action Sequence Tokenization** (arxiv 2501.09747)

연속 action을 이산 토큰으로 변환하는 방법. π0.5 사전학습에 사용.

### 변환 파이프라인

```
연속 action → 정규화 → DCT 변환 → 양자화 → 평탄화 → BPE 압축 → 이산 토큰
```

1. **정규화**: 학습 데이터 1~99 퍼센타일이 [-1, 1]에 매핑
2. **DCT 변환**: 시간 도메인 → 주파수 도메인. 로봇 움직임은 저주파가 대부분 → 대부분 계수가 0에 가까움
3. **양자화**: DCT 계수를 정수로 반올림 → 희소 행렬
4. **평탄화**: 저주파 성분 우선으로 1D 벡터 변환
5. **BPE 압축**: 어휘 크기 1024로 최종 밀집 토큰 생성

학습된 코드북 없는 **분석적 방법** (VQ-VAE 아님). 유일한 학습 요소는 BPE 어휘.

### 압축률 (1초 청크당 토큰 수)

| 데이터셋 | 제어 주파수 | Naive 토큰 | FAST 토큰 | 압축률 |
|---------|-----------|-----------|---------|------|
| BridgeV2 | 5Hz | 35 | 20 | 1.75x |
| DROID | 15Hz | 105 | 29 | 3.6x |
| Table Bussing | 20Hz | 140 | 28 | 5.0x |
| T-Shirt Folding | 50Hz | 700 | 53 | **13.2x** |

고주파일수록 압축 효과가 폭발적으로 증가.

### π0.5에서 FAST를 쓰는 이유

| 단계 | 방식 | 이유 |
|------|------|------|
| Pre-training | FAST (이산 토큰) | 언어/이미지/로봇 action을 동일한 next-token prediction으로 통합. 훈련 5배 빠름, 수렴 3배 빠름 |
| Fine-tuning | Flow Matching 추가 | 연속 action의 정밀도 회복. 추론 속도 750ms → ~100ms |
